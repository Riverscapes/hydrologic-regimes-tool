...
def makeReaches(streamNetwork, dem, marchPrecip, janTemp, aprilTemp, minWinterTemp, tempData, testing):
    ...
    centroid_save = os.path.join(tempData, "Centroid.shp")
    centroid_points = arcpy.FeatureToPoint_management(streamNetwork, centroid_save, "INSIDE")

    extract_string = "{} march;{} jan;{} april;{} winter;{} elev"\
        .format(marchPrecip, janTemp, aprilTemp, minWinterTemp, dem)

    arcpy.gp.ExtractMultiValuesToPoints_sa(centroid_points, extract_string, "BILINEAR")
    fields_list = ["march", "jan", "april", "winter", "elev"]

    point_cursor = arcpy.da.SearchCursor(centroid_points, fields_list)
    line_cursor = arcpy.da.SearchCursor(streamNetwork, "SHAPE@")
    ...
    for row, line in zip(point_cursor, line_cursor):
        i += 1
        if i%10000 == 0:
            arcpy.AddMessage("Creating Reach " + str(i) + " out of " + numReachesString
                         + " (" + str((float(i)/float(numReaches))*100) + "% complete)")
        classification = findClassification(row[0], row[1], row[2], row[3], row[4])
        reach = ClassificationReach(line[0], classification)
        reaches.append(reach)
...
def findClassification(march_precip, jan_temp, april_temp, winter_temp, elevation):

    if march_precip >= 261.7:
        if elevation < 618: # Finds elevation, branches
            return "Rainfall"
        else:
            return "Rain-Snow"
    else:
        if march_precip < 185.6:
            if jan_temp >= -5: # Finds temp in January, branches based on that
                if april_temp < 6.26: # Finds temp in April, branch based on that
                    return "Groundwater"
                else:
                    return "Snow-Rain"
            else:
                if winter_temp < -7.7: # Finds winter temperature, branches based on that
